...
def update_tourist_data_remote(params):
    ...
    try:
        day, agents_static_to_sync, departing_tourist_agent_ids, log_file_name, process_index = params
        ...
        print("asts {0}, dep {1}".format(str(len(agents_static_to_sync)), str(len(departing_tourist_agent_ids))))
        ...
        print("worker {0}, staticagents {1}".format(str(dask_worker.id), str(len(agents_static.shm_age))))

        for agentid in departing_tourist_agent_ids:
