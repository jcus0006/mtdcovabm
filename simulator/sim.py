...
params = {  "popsubfolder": "1kagents4ktourists2019_decupd_v4", # empty takes root (was 500kagents2mtourists2019_decupd_v4 / 100kagents400ktourists2019_decupd_v4 / 10kagents40ktourists2019_decupd_v4 / 1kagents4ktourists2019_decupd_v4)
            ...
            "logmemoryinfo": True,
            "logfilename": "memanalysis_spstrategy.txt" # dask_fs_10k_365d_1n_8w_regression.txt
        }
...
def main():
    ...
    memory_graph_file_name = os.path.join(log_subfolder_path, params["logfilename"].replace(".txt", "_mem_graph.png"))
    memory_analysis_file_name = os.path.join(log_subfolder_path, params["logfilename"].replace(".txt", "_mem_analysis.csv"))
    ...
    mem_analyser = MemoryAnalysis(params["logmemoryinfo"], params["use_mp"], memory_analysis_file_name, memory_graph_file_name)

    try:
        if params["use_mp"] and params["numprocesses"] > 1:
            with mem_analyser.sample("config"):   
                manager = mp.Manager()
            ...
            # with ms.sample(params["logfilename"].replace(".txt", "") + "_init"):   
            with mem_analyser.sample("config"):   
        ...
        # with ms.sample(params["logfilename"].replace(".txt", "") + "_init2"):     
        with mem_analyser.sample("init"):
        ...
        for day in simdays_range: # 365 + 1 / 1 + 1
            # with ms.sample(params["logfilename"].replace(".txt", "") + "_day_" + str(day)): 
            with mem_analyser.sample(f"day_{day}"):    
                ...
                if params["logmemoryinfo"]:
                    ...
                    memory_sums, mem_logs_df = calculate_memory_info(day, params["logmemoryinfo"], it_agents, agents_epi, vars_util, memory_sums, f, mem_logs_df, params["dask_full_stateful"])
...
def calculate_memory_info(day, log_memory_info, it_agents, agents_epi, vars_util, sums=None, f=None, df=None, skip_it=False):
    if log_memory_info:
        start = time.time()
        ...
        if not skip_it:
            ...
            it_agents_mem = 0
            working_schedule_mem = 0
            it_mem = 0
            it_nd_mem = 0
            ndar_mem = 0
            pd_ndar_mem = 0
            tour_ids_mem = 0
            initial_tourist_mem = 0
