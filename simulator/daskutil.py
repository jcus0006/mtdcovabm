...
def handle_futures(method_type: MethodType, day, futures, it_agents, agents_epi, vars_util, task_results_stack_trace_log_file_name, extra_params=False, log_timings=False, dask_full_array_mapping=False, f=None, workers_remote_time_taken=None, processes_remote_time_taken=None, dask_workers_time_taken_keys=None):       
    ...
    for future in results:
        ...
        if type(result) != dict: # success
            if method_type == MethodType.ItineraryMP or method_type == MethodType.ItineraryDist:
                remote_worker_index, it_agents_partial_result, agents_epi_partial_result, vars_util_partial_result, interventions_totals_partial_result, _, _, _, _, remote_time_taken = result
            elif method_type == MethodType.ItineraryDistMP:
                remote_worker_index, it_agents_partial_result, agents_epi_partial_result, vars_util_partial_result, interventions_totals_partial_result, remote_time_taken = result
            ...
            if method_type == MethodType.ItineraryMP or method_type == MethodType.ItineraryDist or method_type == MethodType.ItineraryDistMP: 
                it_agents, agents_epi, vars_util, interventions_totals = sync_results_it(day, it_agents, agents_epi, vars_util, interventions_totals, it_agents_partial_result, agents_epi_partial_result, vars_util_partial_result, interventions_totals_partial_result, remote_worker_index, remote_time_taken, f)
            elif method_type == MethodType.ContactNetworkMP or method_type == MethodType.ContactNetworkDist or method_type == MethodType.ContactNetworkDistMP:
    ...
    return it_agents, agents_epi, vars_util, interventions_totals, workers_remote_time_taken, processes_remote_time_taken
...
def handle_futures_batches(day, futures, agents, agents_epi, vars_util, task_results_stack_trace_log_file_name, extra_params=False, log_timings=False, dask_full_array_mapping=False):
            ...
            for future, result in batch:
                try:
                    ...
                    if not extra_params:
                        ...
                        _, agents_dynamic_partial_result, agents_epi_partial_result, vars_util_partial_result, interventions_totals_partial_result, _, _, _, _ = result

                    worker_process_index = -1
                    if log_timings:
                        worker_process_index = future_count
                        
                    agents, agents_epi, vars_util, interventions_totals = sync_results_it(day, agents, agents_epi, vars_util, interventions_totals, agents_dynamic_partial_result, agents_epi_partial_result, vars_util_partial_result, interventions_totals_partial_result, worker_process_index)
    ...
    return agents, agents_epi, vars_util, interventions_totals
...
def sync_results_it(day, agents, agents_epi, vars_util, interventions_totals, agents_partial, agents_epi_partial, vars_util_partial, interventions_totals_partial, worker_process_index=-1, remote_time_taken=None, f=None):
