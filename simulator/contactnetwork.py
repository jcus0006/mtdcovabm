...
class ContactNetwork:
    def __init__(self, 
        ...
        self.epi_util = Epidemiology(epidemiologyparams, n_locals, n_tourists, locals_ratio_to_full_pop, agents_mp, tourists_active_ids, cells_households, cells_institutions, cells_accommodation, dynparams, sync_queue)
    ...
    def simulate_contact_network(self, day, weekday):        
        ...
        print("simulate_contact_network for simday " + str(day) + ", weekday " + str(weekday) + ", time taken: " + str(time_taken) + ", avg time taken: " + str(avg_time_taken) + ", process index: " + str(self.process_index) + ", end time: " + str(end_time))
        ...
        if len(agents_potentialcontacts) > 0:
            ...
            for agentindex, agentid in enumerate(agents_ids):
                ...
                if agent_potentialcontacts_count > 0:
                    ...
                    agents_degrees[agentindex] = avg_contacts_by_age_activity * timestep_multiplier * potential_contacts_count_multiplier * agent_soc_rate * (1 - self.epi_util.dyn_params.masks_hygiene_distancing_multiplier)
