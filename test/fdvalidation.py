...
class fdvalidation:
    ...
    def __init__(self, freqdistname, freqdist, expectedvals, actualvals, diffvals, femexpectedvals, femactualvals, femdiffvals, femaledist, secactualvals=None, secdiffvals=None):
        ...
        self.sec_actual_values = secactualvals
        self.sec_difference_values = secdiffvals
...
def display_male_female_expected_vs_actual(propName, labelName, expected_values, actual_values, fem_expected_values, fem_actual_values):
    ...
    ax.set_title('Distribution of Ages (Male)')
    ...
    ax.set_xlabel("Ages")
    ax.set_ylabel('Frequency')
    ax.set_title('Distribution of Ages (Female)')
...
def create_smoothed_age_dist_tourists(age_groups):
    total_population = sum(group[2] for group in age_groups)
    age_distribution = {}
    for group in age_groups:
        start_age, end_age, percentage, _ = group
        num_years = end_age - start_age + 1
        # assume uniform distribution as we don't have further information
        yearly_percentage = (percentage / total_population) / num_years
        for year in range(start_age, end_age + 1):
            age_distribution[year] = yearly_percentage
...
population_sub_folder = "500kagents2mtourists2019_decupd_v4" # 500kagents2mtourists2019_decupd_v4 / 10kagents40ktourists2019_decupd_v4

maltafile  = open("./population/" + population_sub_folder + "/Malta.json")
...
if is_locals:
    ...
    # fdlist = [ "enrollment_rates_by_age", 
    #             "employment_status_byage_distributions" ]

    # fdlist = [ "sex_byage_distributions" ]
    ...
    smoothedagedistfile = open("./population/" + population_sub_folder + "/smoothedagedist.json")
    smoothedagedist = json.load(smoothedagedistfile)

    if not forceload and os.path.exists("./population/" + population_sub_folder + "/fdvalidations.json"):
        fdvalidationsfile = open("./population/" + population_sub_folder + "/fdvalidations.json")
        ...
        for index, fd in enumerate(fdlist):
            ...
            if fd == "population_age_distributions": # straight ages 1 to 100
                ...
                expectedvals = {i:0 for i in range(5, 16, 1)}
                ...
                expectedvals = {i:0 for i in range(101) if 15 < i <= 65}
                fem_expectedvals = {i:0 for i in range(101) if 15 < i <= 65}
    ...
    for fdval in fdvalidations:
        ...
        if frequencydistributionname == "population_age_distributions":
            ...
            agents_by_enrolled = [1 if v["scid"] is not None else 0 for v in agents.values() if 5 <= v["age"] <= 15]
            ...
            compare_category_ageranges_expected_vs_actual(normal_exp_hist, normal_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Male Normal Category'", (8, 5), 4)
            compare_category_ageranges_expected_vs_actual(overweight_exp_hist, overweight_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Male Overweight Category'", (8, 5), 4)
            compare_category_ageranges_expected_vs_actual(obese_exp_hist, obese_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Male Obese Category'", (8, 5), 4)

            compare_category_ageranges_expected_vs_actual(fem_normal_exp_hist, fem_normal_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Female Normal Category'", (8, 5), 4)
            compare_category_ageranges_expected_vs_actual(fem_overweight_exp_hist, fem_overweight_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Female Overweight Category'", (8, 5), 4)
            compare_category_ageranges_expected_vs_actual(fem_obese_exp_hist, fem_obese_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Female Obese Category'", (8, 5), 4)
        elif frequencydistributionname == "employment_status_byage_distributions":
            agents_by_empstatus = [v["empstatus"] for v in agents.values() if 15 < v["age"] <= 65]
            ...
            emp_exp_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=emp_exp_counts)
            unemp_exp_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=unemp_exp_counts)
            inact_exp_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=inact_exp_counts)

            emp_act_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=emp_act_counts)
            unemp_act_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=unemp_act_counts)
            inact_act_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=inact_act_counts)

            fem_emp_exp_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=fem_emp_exp_counts)
            fem_unemp_exp_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=fem_unemp_exp_counts)
            fem_inact_exp_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=fem_inact_exp_counts)

            fem_emp_act_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=fem_emp_act_counts)
            fem_unemp_act_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=fem_unemp_act_counts)
            fem_inact_act_hist, _ = np.histogram(np.arange(16, 66, 1), bins=bin_edges, weights=fem_inact_act_counts)

            compare_category_ageranges_expected_vs_actual(emp_exp_hist, emp_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Male Employed Category'", (8, 5), 2, bin_edges[:-1])
            compare_category_ageranges_expected_vs_actual(unemp_exp_hist, unemp_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Male Unemployed Category'", (8, 5), 2, bin_edges[:-1])
            compare_category_ageranges_expected_vs_actual(inact_exp_hist, inact_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Male Inactive Category'", (8, 5), 2, bin_edges[:-1])

            compare_category_ageranges_expected_vs_actual(fem_emp_exp_hist, fem_emp_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Female Employed Category'", (8, 5), 2, bin_edges[:-1])
            compare_category_ageranges_expected_vs_actual(fem_unemp_exp_hist, fem_unemp_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Female Unemployed Category'", (8, 5), 2, bin_edges[:-1])
            compare_category_ageranges_expected_vs_actual(fem_inact_exp_hist, fem_inact_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for 'Female Inactive Category'", (8, 5), 2, bin_edges[:-1])
            ...
            plt.figure(figureindex, figsize=(8, 5))
            ...
            compare_category_industries_expected_vs_actual(ft_exp_counts, ft_act_counts, "Industries", "Counts", "Expected vs Actual Counts for 'Male FullTime Category'", (8, 5), 0.35)
            compare_category_industries_expected_vs_actual(ptft_exp_counts, ptft_act_counts, "Industries", "Counts", "Expected vs Actual Counts for 'Male PartTime (FullTime) Category'", (8, 5), 0.35)
            compare_category_industries_expected_vs_actual(ptnoft_exp_counts, ptnoft_act_counts, "Industries", "Counts", "Expected vs Actual Counts for 'Male PartTime (no FullTime) Category'", (8, 5), 0.35)

            compare_category_industries_expected_vs_actual(fem_ft_exp_counts, fem_ft_act_counts, "Industries", "Counts", "Expected vs Actual Counts for 'Female FullTime Category'", (8, 5), 0.35)
            compare_category_industries_expected_vs_actual(fem_ptft_exp_counts, fem_ptft_act_counts, "Industries", "Counts", "Expected vs Actual Counts for 'Female PartTime (FullTime) Category'", (8, 5), 0.35)
            compare_category_industries_expected_vs_actual(fem_ptnoft_exp_counts, fem_ptnoft_act_counts, "Industries", "Counts", "Expected vs Actual Counts for 'Female PartTime (no FullTime) Category'", (8, 5), 0.35)
            ...
            compare_category_ageranges_expected_vs_actual(emp_exp_hist, emp_act_hist, "Age Ranges", "Counts", "Expected vs Actual Counts for Institution Rates by Age", (8, 5), 4, bin_edges[:-1])
    ...
    tourismdf = maltafds["tourism_distributions"]
    # smoothedagedist = create_smoothed_age_dist_tourists(tourismdf["age"])        

    maletourists = {k:v for k, v in enumerate(tourists) if v["gender"] == 0}
    femaletourists = {k:v for k, v in enumerate(tourists) if v["gender"] == 1}
    ...
    for age in range(101):
        agentsbyage = {k:v for k, v in enumerate(tourists) if v["age"] == age}
        malesbyage = {k:v for k, v in agentsbyage.items() if v["gender"] == 0}
        femalesbyage = {k:v for k, v in agentsbyage.items() if v["gender"] == 1}
    ...
    num_groups_by_accomtype, num_tourists_by_accomtype = {}, {}
    num_groups_by_quarter, num_tourists_by_quarter = {}, {}
    num_groups_by_purpose, num_tourists_by_purpose = {}, {}
    num_groups_by_duration, num_tourists_by_duration = {}, {}
    quarter_thresholds = [91, 181, 271, 365]
    durations_thresholds = [3, 6, 30]
    for tour_group_id, tour_group in enumerate(touristsgroups):
        if tour_group["arr"] > 0:
            num_tourists_in_group = 0
            for subgroup in tour_group["subgroupsmemberids"]:
                num_tourists_in_group += len(subgroup)
            if tour_group["accomtype"] not in num_groups_by_accomtype:
                num_groups_by_accomtype[tour_group["accomtype"]] = 0
            if tour_group["accomtype"] not in num_tourists_by_accomtype:
                num_tourists_by_accomtype[tour_group["accomtype"]] = 0

            num_groups_by_accomtype[tour_group["accomtype"]] += 1
            num_tourists_by_accomtype[tour_group["accomtype"]] += num_tourists_in_group

            quarter = -1
            for index, quarter_max in enumerate(quarter_thresholds):
                if tour_group["arr"] <= quarter_max:
                    quarter = index
                    break

            if quarter not in num_groups_by_quarter:
                num_groups_by_quarter[quarter] = 0
            if quarter not in num_tourists_by_quarter:
                num_tourists_by_quarter[quarter] = 0

            num_groups_by_quarter[quarter] += 1
            num_tourists_by_quarter[quarter] += num_tourists_in_group

            if tour_group["purpose"] not in num_groups_by_purpose:
                num_groups_by_purpose[tour_group["purpose"]] = 0
            if tour_group["purpose"] not in num_tourists_by_purpose:
                num_tourists_by_purpose[tour_group["purpose"]] = 0

            num_groups_by_purpose[tour_group["purpose"]] += 1
            num_tourists_by_purpose[tour_group["purpose"]] += num_tourists_in_group

            duration = -1
            for index, duration_max in enumerate(durations_thresholds):
                if tour_group["dep"] - tour_group["arr"] <= duration_max:
                    duration = index
                    break

            if duration not in num_groups_by_duration:
                num_groups_by_duration[duration] = 0
            if duration not in num_tourists_by_duration:
                num_tourists_by_duration[duration] = 0

            num_groups_by_duration[duration] += 1
            num_tourists_by_duration[duration] += num_tourists_in_group

            n_groups += 1
    
    num_groups_by_accomtype, num_tourists_by_accomtype = dict(sorted(num_groups_by_accomtype.items())), dict(sorted(num_tourists_by_accomtype.items()))
    num_groups_by_quarter, num_tourists_by_quarter = dict(sorted(num_groups_by_quarter.items())), dict(sorted(num_tourists_by_quarter.items()))
    num_groups_by_purpose, num_tourists_by_purpose = dict(sorted(num_groups_by_purpose.items())), dict(sorted(num_tourists_by_purpose.items()))
    num_groups_by_duration, num_tourists_by_duration = dict(sorted(num_groups_by_duration.items())), dict(sorted(num_tourists_by_duration.items()))
    ...
    for index, fd in enumerate(fdlist):
        ...
        if fd == "age": # ages by age_brackets based on input params
            freqdist = mtfd[fd]

            expectedvals = {(min_age, max_age):0 for min_age, max_age, _, _ in freqdist} 

            for age_bracket_index, _ in enumerate(expectedvals):
                min_age, max_age, percentage, _ = freqdist[age_bracket_index]

                actual_agents_in_age = 0
                for age in range(min_age, max_age + 1, 1):
                    agentsbyage = agentsbyages[age]
                    actual_agents_in_age += len(agentsbyage)
                    
                expectedvals[(min_age, max_age)] = round(n * percentage)

                actualvals[(min_age, max_age)] = actual_agents_in_age

                differencevals[(min_age, max_age)] = expectedvals[(min_age, max_age)] - actualvals[(min_age, max_age)]
        elif fd == "gender":
            freqdist = mtfd[fd]

            expectedvals = {i:round(percentage * n) for i, percentage in enumerate(freqdist)}
            actualvals = {0:len(maletourists), 1:len(femaletourists)}

            for gender in expectedvals.keys():
                differencevals[gender] = expectedvals[gender] - actualvals[gender]
        elif fd == "accommodation_type":
            freqdist = mtfd[fd]
            expectedvals = {accom_type_id:round(n_groups*percentage) for accom_type_id, percentage in freqdist} 
            actualvals = num_groups_by_accomtype

            for accom_type_id in expectedvals.keys():
                differencevals[accom_type_id] = expectedvals[accom_type_id] - actualvals[accom_type_id]
        elif fd == "quarter":
            freqdist = mtfd[fd]
            expectedvals = {index:round(n_groups*percentage) for index, (_, _, percentage) in enumerate(freqdist)} 
            actualvals = num_groups_by_quarter

            for accom_type_id in expectedvals.keys():
                differencevals[accom_type_id] = expectedvals[accom_type_id] - actualvals[accom_type_id]
        elif fd == "purpose":
            freqdist = mtfd[fd]
            expectedvals = {id:round(n_groups*percentage) for index, (id, percentage) in enumerate(freqdist)} 
            actualvals = num_groups_by_purpose

            for purpose in expectedvals.keys():
                differencevals[purpose] = expectedvals[purpose] - actualvals[purpose]
        elif fd == "group_size":
            pass
        elif fd == "duration":
            freqdist = mtfd[fd]
            expectedvals = {index:round(n_groups*percentage) for index, (_, _, percentage, _) in enumerate(freqdist)} 
            actualvals = num_groups_by_duration

            for duration_bracket in expectedvals.keys():
                differencevals[duration_bracket] = expectedvals[duration_bracket] - actualvals[duration_bracket]

        fdval = fdvalidation(fd, freqdist, expectedvals, actualvals, differencevals, fem_expectedvals, fem_actualvals, fem_differencevals, femalefreqdist, secactualvals, secdifferencevals)
        fdvalidations.append(fdval)
    ...
    for fdval in fdvalidations:
        ...
        expected_values = fdval["expected_values"] if type(fdval) is dict else fdval.expected_values
        actual_values = fdval["actual_values"] if type(fdval) is dict else fdval.actual_values
        sec_actual_values = fdval["sec_actual_values"] if type(fdval) is dict else fdval.sec_actual_values

        if frequencydistributionname == "age":
            labels = [f"{k[0]}-{k[1]}" for k in expected_values.keys()]
            expected_vals = list(expected_values.values())
            ...
            ax.bar(r1, expected_vals, width=barWidth, edgecolor="grey", label="Expected Counts")
            ax.bar(r2, actual_vals, width=barWidth, edgecolor="grey", label="Actual Counts")
            
            ax.set_xlabel("Age Bracket")
            ax.set_ylabel("Frequency")
            ax.set_title("Expected vs Actual Population by Age Bracket")
            ...
            ax.legend()
            plt.show()
        elif frequencydistributionname == "gender":
            labels = ["Male", "Female"]
            expected_vals = list(expected_values.values())
            actual_vals = list(actual_values.values())
            ...
            ax.set_ylabel("Frequency")
            ax.set_title("Expected vs Actual Tourists by Gender")
            ...
            ax.legend()
            plt.show()
        elif frequencydistributionname == "accommodation_type":
            labels = ["Hotel", "Guest House", "Tourist Village", "Self Catering"]
            expected_vals = list(expected_values.values())
            actual_vals = list(actual_values.values())
            ...
            ax.bar(r1, expected_vals, width=barWidth, edgecolor="grey", label="Expected Counts")
            ax.bar(r2, actual_vals, width=barWidth, edgecolor="grey", label="Actual Counts")
            # ax.bar(r3, sec_actual_vals, width=barWidth, edgecolor="grey", label="Actual Counts")
            
            ax.set_xlabel("Accommodation Type")
            ax.set_ylabel("Frequency (Groups)")
            ax.set_title("Expected vs Actual Tourists Groups by Accommodation Type")
            ...
            ax.legend()
            plt.show()
        elif frequencydistributionname == "quarter":
            ...
            ax.set_ylabel("Frequency (Groups)")
            ax.set_title("Expected vs Actual Tourists Groups by Quarter")
            ...
            ax.legend()
            plt.show()
        elif frequencydistributionname == "purpose":
            labels = ["Holiday", "Business", "Family Visit", "Other"]
            expected_vals = list(expected_values.values())
            actual_vals = list(actual_values.values())
            ...
            ax.set_ylabel("Frequency (Groups)")
            ax.set_title("Expected vs Actual Tourists Groups by Purpose")
            ...
            ax.legend()
            plt.show()
        elif frequencydistributionname == "group_size":
            pass
        elif frequencydistributionname == "duration":
            labels = ["1-3", "4-6", "7-30"]
            expected_vals = list(expected_values.values())
            actual_vals = list(actual_values.values())
            ...
            ax.set_xlabel("Duration Brackets")
            ax.set_ylabel("Frequency (Groups)")
            ax.set_title("Expected vs Actual Tourists Groups by Duration Brackets")
